    ...
    public class RemoteLRS : ILRS
    {
        ...
        private string AppendParamsToExistingQueryString(string currentQueryString, IEnumerable<KeyValuePair<string, string>> parameters)
        {
            foreach (KeyValuePair<String, String> entry in parameters)
            {
                ...
                currentQueryString += WebUtility.UrlEncode(entry.Key) + "=" + WebUtility.UrlEncode(entry.Value);
            }
        ...
        private MyHTTPResponse MakeSyncRequest(MyHTTPRequest req)
        {
            ...
            if (req.content != null)
            {
                ...
                webReq.Timeout = 3000; // 1000 ms or 1 s
                try
                {
                    using (var stream = webReq.GetRequestStream())
                    {
                        stream.Write(req.content, 0, req.content.Length);
                        AnalyticsManager.analyticsManager.ClearStatements(); // Clearing out the list
                        SaveData.Instance.RemoveFile(); // Deleting file
                        Debug.Log("Uploading data to server");
                    }
                }
                catch (Exception e) // if there is an exception we will save the list to a json
                {
                    Debug.LogError("Web Request to the LRS timed out, Writing the data to a file .....  \n" + e.ToString());
        ...
        public StatementsResultLRSResponse SaveStatements(List<Statement> statements)
        {
            ...
            {
                if (i < statements.Count) // Added the if here to double check (Done by shaun)
                {
